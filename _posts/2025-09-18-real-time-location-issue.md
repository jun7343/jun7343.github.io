...
실시간 시각화가 목적이라 영속화가 필수는 아니므로 비효율적입니다. 추가로 다음 문제가 있습니다.

- 조회 기준 시점이 모호함  
- 중복 조회 방지를 위한 오프셋/커서 관리 필요  
- 지속적인 INSERT·SELECT로 DB I/O 부하 증가  
- 복제 지연(Primary/Replica)으로 실시간성 저하

따라서 실시간 위치 스트림에는 부적합하다고 판단했습니다.
