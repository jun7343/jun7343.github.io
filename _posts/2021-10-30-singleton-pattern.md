...
- **Eager Initialization**  
  이른 생성 방법으로써 애플리케이션이 로드될 때 초기화 하여 사용하는 방법이다. 로드 되는 시점에 미리 초기화를 함으로써 멀티 스레드 환경에서 발생할 수 있는 인스턴스 중복 생성을 방지할 수 있다는 장점이 있다. 하지만 애플리케이션이 로드되는 시점에 초기화하는 만큼 해당 인스턴스이 무겁고 잘 사용하지 않는 인스턴스라면 초기화 함으로써 메모리 낭비라는 비효율적인 결과를 초래할 수 있다.
