...
덧붙여 Intel은 Hyper Threading 기술을 위해 CPU에서 자주 사용되는 부분 즉 레지스터(called architectural states)를 복제하게되어 CPU 크기가 기존 CPU 크기보다 5% 증가하였다고 한다.  
또한 Intel이 주장하는 30%의 성능 효율은 Intel Socket의 수마다 상이할 수 있다. Single Socket일때 최대 30% 효율 상승이며, Dual Socket일 경우 최대 15%까지 효율을 올릴 수 있다. 그 이상의 Socket의 경우 Test를 걸쳐 효율성을 알아봐야 한다.
...
이러한 명령어 사이클이 돌때 Thread는 온전히 일하는것이 아니라 유휴 상태에 빠질때가 있는데 이때 다른 코어가 유휴 상태에 빠진 코어의 CPU 리소스를 넘겨 받아 명령어를 수행함으로써 효율을 높일 수가 있다.
...
위의 이미지를 보면 빠른 이해가 될거라 생각된다. 이렇게 Intel Hyper Threading 기술을 사용함으로써 Multi Tasking이 가능하게끔 할 수 있던것이다.
...
이제야 Hyper Thread에 대해 설명했으며 친구가 말했던 Process의 Thread 역시 제대로 학습하여 기록해야 한다. 하지만 Process의 Thread를 학습하기 전에 Process를 학습해야 한다.
